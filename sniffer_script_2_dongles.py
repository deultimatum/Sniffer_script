...
dongle_2 = sniffer_dongle[1] if len(sniffer_dongle) > 1 else None
if dongle_2:
    print("dongle 2:", dongle_2)


def run_sniffer(dongle_dsn_1, dongle_dsn_2):
    """runs wireshark using 2 dongles"""
    results = subprocess.run(["sudo", "wireshark", "-i", dongle_dsn_1, "-i", dongle_dsn_2, "-k", "-S", "-l"])
    if results.returncode != 0:
...
channel_1 = input("enter channel 2G : ")
channel_2 = input("enter channel 5G:  ")
processes = []
for dongle_dsn, channel, bw in [(dongle_1, channel_1, "HT20"), (dongle_2, channel_2, "HT20")]:
